...
public class WxScheduler {
    ...
    @Autowired
    private IWxJobLogManager wxJobManager;
    ...
    private void processJms(String key, String json) {
        switch (key) {
            case RedisKey.WAIT_CONSUME_FUNCTION_CODE:
                WxJobLog wxJob = JSON.parseObject(json, WxJobLog.class);
                LOG.info("Consume job functionCode [" + wxJob.getFunctionCode() + "] start...");
                ExecutionContext.setUserId(wxJob.getCreateUser());
                try {
                    wxJob.setExecuteStatus(WxJobLog.ExecuteStatus.SUCCESS);
                } catch (Exception e) {
                    LOG.error(e);
                    wxJob.setExecuteStatus(WxJobLog.ExecuteStatus.FAILED);
